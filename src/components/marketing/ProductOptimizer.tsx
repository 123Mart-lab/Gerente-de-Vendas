...
export default function ProductOptimizer() {
  ...
  // Fetch initial filters from server
  useEffect(() => {
    axios.get('/api/marketing/seo-filters').then(res => {
      if (res.data) {
        setSeoFiltersEnabled(res.data.enabled);
        seoFiltersEnabledRef.current = res.data.enabled;
        setSeoScoreMin(res.data.min);
        seoScoreMinRef.current = res.data.min;
        setSeoScoreMax(res.data.max);
        seoScoreMaxRef.current = res.data.max;
        setIgnoreKits(res.data.ignoreKits);
        ignoreKitsRef.current = res.data.ignoreKits;
        setIgnoreAlteredCondition(res.data.alteredCondition);
        ignoreAlteredConditionRef.current = res.data.alteredCondition;
        setIgnoreAlteredDays(res.data.alteredDays);
        ignoreAlteredDaysRef.current = res.data.alteredDays;
      }
    }).catch(console.error);
  }, []);
